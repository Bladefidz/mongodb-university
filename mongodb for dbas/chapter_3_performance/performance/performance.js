...
homework.a = function () {
    ...
    var b = new BinData(0, "abcdefgh");
    return b.length() + b.subtype();
}
homework.c = function () {
    var long = false;
    var y = 0;
    ...
    if (!k) {
        print("not seeing the expected activity, is homework.b() really running in a separate shell? try again.");
        return;
    }
    ip.forEach(function (x) {
        if (x.op == 'update' && x.secs_running > 3) y += x.secs_running;
    });
    return y ^ 0xc;
}
homework.d = function () {
    ...
    var t = db.sensor_readings;
    var s = t.stats();
    return Math.round((s.size + s.indexSizes._id_) / 100000);
}
homework.b = function () {
    ...
    new Wrapper(function () {
        ...
        while (1) {
            t2.find().skip(Math.floor(Math.random() * 20000)).limit(400).toArray();
        }
    }).start();
    new Wrapper(function () {
